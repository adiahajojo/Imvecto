...
export async function POST(request: Request) {
  ...
  try {
    ...
    const prepared = await brickkenPrepare("approve", {
      chainId: "aa36a7",
      signerAddress: investorAddress.trim().toLowerCase(),
      tokenSymbol: "USDT",
      spenderAddress: tokenizerInfo.escrowAddress.trim().toLowerCase(),
      tokenizerAddress: tokenizerInfo.companyWalletAddress.trim().toLowerCase(),
